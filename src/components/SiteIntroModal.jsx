...
export default function SiteIntroModal() {
  ...
  useEffect(() => {
    ...
    const loadIntro = async () => {
      ...
      if (window.localStorage.getItem('harmonee_intro_seen') !== nextVersion) {
        setIsOpen(true);
      }
    };
    loadIntro();
  ...
  const closeIntro = useCallback(() => {
    window.localStorage.setItem('harmonee_intro_seen', version);
    setIsOpen(false);
  }, [version]);

  useModalHistory(isOpen, closeIntro);
  ...
  return (
    <div
      className="fixed inset-0 z-[500] flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm sm:p-4 modal-backdrop"
      onClick={closeIntro}
    >
      <div
        className="relative w-full sm:max-w-md bg-white sm:rounded-2xl rounded-t-2xl shadow-2xl overflow-hidden modal-panel-bottom"
      ...
      >
        ...
        <button
          type="button"
          onClick={closeIntro}
          className="absolute right-4 top-4 z-10 w-10 h-10 flex items-center justify-center rounded-xl text-muted hover:text-text-main hover:bg-surface-alt transition-colors"
          aria-label="Tutup"
        >
          <X size={18} />
        </button>
        <div className="p-6 sm:p-8 text-center">
          ...
          <button
            type="button"
            onClick={closeIntro}
            className="w-full h-12 text-sm font-semibold text-cream bg-primary rounded-xl hover:bg-primary-hover active:scale-[0.98] transition-all"
          >
            {intro.buttonLabel}
          </button>
        </div>
