  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm sm:p-4 modal-backdrop" onClick={onClose}>
      <div className="bg-white w-full sm:max-w-2xl sm:rounded-2xl rounded-t-2xl max-h-[92vh] sm:max-h-[88vh] overflow-y-auto relative shadow-2xl modal-panel-bottom" onClick={e => e.stopPropagation()}>
        ...
        {/* Close button */}
        <button onClick={onClose} className="absolute top-4 right-4 z-10 w-9 h-9 flex items-center justify-center rounded-full bg-black/30 text-white hover:bg-black/50 transition-colors">
          <X size={16} />
        </button>
        ...
        <div className="p-5 sm:p-6 space-y-5 modal-content">
          ...
          <div>
            ...
            <div className="mt-2.5 flex flex-wrap items-center gap-2">
              ...
              {shop.area && <span className="text-xs font-medium text-primary bg-cream px-2.5 py-1 rounded-lg">{shop.area}</span>}
            </div>
            {shop.tags?.length > 0 && (
              <div className="mt-3 flex flex-wrap gap-1.5">
                {shop.tags.map((tag, i) => <span key={i} className="text-xs text-muted bg-surface-alt px-2.5 py-1 rounded-lg">{tag}</span>)}
              </div>
            )}
          </div>
